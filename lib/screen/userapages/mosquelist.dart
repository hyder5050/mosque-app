...
class _MosquelistState extends State<Mosquelist>
  ...
  void _filterMosques() {
    ...
    setState(() {
      if (query.isEmpty) {
        _filteredMosques = List.from(provider.mosques);
      } else {
        _filteredMosques = provider.mosques.where((mosque) {
          return mosque.name.toLowerCase().contains(query) ||
              mosque.address.toLowerCase().contains(query);
        }).toList();
      }
  ...
  void _showOnAppMap(Mosque mosque) {
    // Switch to map tab and pass the mosque location
    // This will be handled in userbottombar
    Navigator.pop(context);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<MosqueProvider>(
      builder: (context, provider, child) {
        // Update filtered mosques when provider data changes
        if (_filteredMosques.isEmpty && provider.mosques.isNotEmpty) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            _filterMosques();
          });
        }
              ...
              child: CustomScrollView(
                ...
                background: Container(
                  ...
                  SliverToBoxAdapter(
                    child: Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          Text(
                            'Showing ${_filteredMosques.length} of ${provider.mosques.length} mosques',
                            style: TextStyle(
                              color: Colors.grey[600],
                              fontSize: width * 0.032,
                            ),
                  ...
                  if (provider.isLoading)
                    ...
                    SliverFillRemaining(
                      child: _buildErrorState(width, height, provider),
                    )
                  else if (_filteredMosques.isEmpty)
                    SliverFillRemaining(child: _buildEmptyState(width, height))
                  else if (_isGridView)
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: height * 0.03),
            ElevatedButton.icon(
              onPressed: () => provider.loadMosques(forceRefresh: true),
              icon: const Icon(Icons.refresh_rounded),
              label: const Text('Try Again'),
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.green.shade600,
